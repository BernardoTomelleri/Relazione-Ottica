...
b = \sin{i} = n \sin{r}
\end{equation}
Dal sistema formato dalle equazioni \eqref{eq:snell} e \eqref{eq: k_defl} si ottiene un'espressione in $i$ per l'apertura angolare $\delta_k = \pi - \theta_k$
...
\subsection{Legge di Cauchy}
Per stimare il coefficiente $A$ della legge di Cauchy \ref{eq:Cauchy} si è ripetuto lo stesso fit distinguendo nei rapporti dei raggi le bande del medesimo colore:
\begin{figure}[H]
	\centering
		\scalebox{0.8}{\input{./figs/cauchy.pgf}}
	\caption{Fit con la legge di Cauchy \label{fig:cauchy}}
\end{figure}
da cui si trovano i valori
\begin{align}\label{aln:cauchy}
n_0 &= 1.318 \pm 0.005 \\
A &= 5.3 \pm 0.3 \; \times 10^{-3} \\
\chi^2 &= 0.5/2
\end{align}
Questi risultano compatibili con quanto trovato da \cite{cauchy}. 
\section{Conclusioni}
...
\delta = i - \phi + \arcsin{\left( \sin{\phi} \sqrt{n^2 - \sin{i}} - \sin^2{i} \cos{\phi} \right)}
\end{equation}
...
\cos\alpha = \sin\theta_1\sin\theta_2 + \cos\theta_1\cos\theta_2 \cos{(\phi_1 - \phi_2)}
\end{equation}
